...
namespace Simpl.Expenses.WebAPI.Tests
{
    public class UsersControllerTests : IntegrationTestBase
    {
        ...
        public async Task Put_UpdateExistingUser_ReturnsNoContent()
        {
            ...
            // Act
            var response = await _client.PutAsJsonAsync($"/api/users/{user.Id}", updateUser);
        ...
        public async Task GetAll_WithMultipleUsers_ReturnsAllUsers()
        {
            ...
            Assert.True(returnedUsers.Count >= 3, "Expected at least 3 users to be returned");
        ...
        {
            // Arrange
            var deletedUsers = await DeleteAllAsync<User>();
            // Act
            var response2 = await _client.GetAsync("/api/users");

            // Assert
            response2.EnsureSuccessStatusCode();
            var returnedUsers = await response2.Content.ReadFromJsonAsync<List<UserDto>>();

            Assert.NotNull(returnedUsers);
            Assert.Empty(returnedUsers);

            // Restore deleted users for other tests
