...
namespace Simpl.Expenses.Infrastructure.Persistence.Configurations
{
    public class PurchaseOrderDetailConfiguration : IEntityTypeConfiguration<PurchaseOrderDetail>
    {
        public void Configure(EntityTypeBuilder<PurchaseOrderDetail> builder)
        {
            ...
            builder.HasOne(pod => pod.CostCenter).WithMany().HasForeignKey(pod => pod.CostCenterId).OnDelete(DeleteBehavior.NoAction);
            builder.HasOne(pod => pod.AccountProject).WithMany().HasForeignKey(pod => pod.AccountProjectId).OnDelete(DeleteBehavior.NoAction);
